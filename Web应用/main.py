...
# 配置日志基本信息
# %(asctime)s:日志时间； %(levelname)s:日志级别； %(filename)s:文件名； %(lineno)d:行号； %(message)s:日志信息
logging.basicConfig(
    level=logging.INFO, # 设置日志级别为INFO (级别有：DEBUG、INFO、WARNING、ERROR、CRITICAL)
...
def chat(request: ChatRequest) -> ApiResponse:
    ...
    completion = client.chat.completions.create(
        ...
        messages=messages,
        stream=False,
        temperature=1.5 # 模型生成结果随机性
    )

    # 4. 获得响应数据
    ai_response = completion.choices[0].message.content
    logging.info(f"<---- AI大模型响应的会话信息：{ai_response}")
...
def get_sessions() -> ApiResponse:
    ...
    session_ids.sort(reverse=True) # 按时间排序

    # 3. 返回数据
    return ApiResponse(code=200, message="获得会话列表成功", data=session_ids)


# 获取指定会话信息 -> /api/sessions/2026-01-01_00-00-00 (路径参数)
...
# 删除指定会话信息 -> /api/sessions/2026-01-01_00-00-00 (路径参数)
@app.delete("/api/sessions/{session_id}")
def delete_session(session_id: str) -> ApiResponse:
    logging.info(f"删除指定会话信息: {session_id}")
    # 1. 获取会话文件名
    session_file = get_session_file_name(session_id)
...
# 异常处理器，用于处理所有异常 -> 返回的对象的类型：Response
